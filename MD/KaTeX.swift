...
public final class MathFontFile {
    ...
    public static func shared() throws -> MathFontFile {
        ...
        if let u = Res.url("STIXTwoMath", "otf", dev: dir) {
            candidates.append(u)
        }
...
enum Alphanumerics {
    ...
    /// Map an ASCII letter or digit into the requested math alphanumeric
    /// block; anything else comes back unchanged.
...
final class Parser {
    ...
    private func textArgument() throws -> String {
        ...
        if eat("{") {
            var depth = 0
            // Gaps run from the brace: `\text{ is even}` keeps its leading
            // space and `\text{if }` its trailing one.
...
public struct MathLayout {
    ...
    /// `at` is the top-left of the bounding box; `flipped` for y-down
    /// contexts; `color` overrides the ink so a theme switch only redraws.
    public func draw(in ctx: CGContext, at point: CGPoint,
                     color ink: CGColor? = nil, flipped: Bool = false) {
        // Drawing fills the font's CTFont cache, so it takes the same lock the
        // layout does.
